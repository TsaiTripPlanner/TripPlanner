...
## 🏗️ 系統架構
- **前端框架**: React 19 (Hooks)
- **樣式工具**: Tailwind CSS(JIT 模式)
- **後端整合**:
  - Firebase Firestore: 實時數據同步，並啟用 persistentLocalCache 確保離線時仍可讀寫。
  - Firebase Auth: 採用「通行碼 (Access Code)」邏輯，簡化群組同步。
- **媒體處理**:
  - Cloudinary API: 雲端圖片存儲與自動縮圖。
  - Browser Image Compression: 上傳前自動執行 Web Worker 壓縮，限制大小於 0.8MB 內。
- **離線支援**: PWA (Service Worker) 具備 Network First 策略，確保在飛機上或海外網路不穩時仍能運作。

---

## 📍 1. 行程規劃 (Itinerary)
- **多天數管理**: 支援動態天數切換，並顯示對應日期與星期。
- **進階排序系統**: 
  1. 優先依照 `startTime` (24小時制時間字串) 升序排列。
  2. 同一時間或未定時間項目，支援手動拖拽排序 (order)。
  3. 技術實作: 搭配 Firestore 複合索引 (Composite Index) 確保查詢效能。
- **自動時長計算**: 輸入開始與結束時間後，自動計算並顯示持續長度(e.g., 2h 30m)。
...
- **編輯功能:** 支援隨時修改分類名稱、項目名稱，具備二次確認刪除機制。
...
- **記帳明細**: 支援輸入細項備註（例如：拉麵 1000, 煎餃 500），點擊可展開查看。
...
- **交通指引 (Transport)**：
  ...
  - 圖像優先：圖片強制使用 object-contain  確保長條狀或比例奇特的交通圖不被裁切。
  - 外部連結：顯眼的寬版按鈕，方便一鍵跳轉至 Google Maps 或鐵路官網。
  - 天數關聯: 交通建議可與特定天數掛鉤，方便過濾當天資訊。
- **景點介紹 (Spot Layout)**:
  - 雜誌感網格: 16:9 比例縮圖，具備視覺重心。
  - 摘要顯示：列表僅顯示「介紹」分頁的前兩行內容（Line-clamp）。
  - 完整詳情視窗 (Detail Modal): 點擊後開啟全螢幕 Modal，包含大圖、五大分頁導覽、Markdown 內容與官網跳轉按鈕。
  - 五大子分頁解析器: 利用自定義 Parser 偵測標籤（[美食]、[特色店家] 等），將長文自動拆解至對應分頁。
...
- **Markdown 智慧文本**：
  ...
  - 網址自動偵測: 說明文字內的 URL 會自動轉換為藍色可點擊連結。
- **編輯工具列**: 提供快捷語法按鈕，點擊後會自動將語法插入游標位置並重新聚焦。
...
- **行動端適配 (Responsive Fixes)**:
  - 針對 iOS 下拉選單、日期輸入框進行樣式修正。
  - 實作「底欄避讓 (Safe Area)」確保 iPhone Home Indicator 不會遮擋導覽列。
  - 所有的 flex 容器內 input 強制加上 min-w-0，防止撐破版面。
- **圖片上傳優化**:

...
針對不同類型的資料提供最適合的視覺呈現。
- **交通指引 (Transport)**：
  - 強化圖片顯示：適合存放乘車時刻表、轉乘地圖截圖。
  - 一鍵跳轉：顯眼的外部地圖/連結按鈕。
- **景點介紹 (Spot Layout)**:
  - 雜誌感網格: 16:9 比例縮圖，具備視覺重心。
  - 富文本詳情: 支援 Markdown 語法（標題 #、粗體 **），並分為「介紹、美食、特色店家、附近景點、注意事項」五大子分頁。
  - 動態展開: 列表僅顯示摘要，詳情採用 Modal 全螢幕彈窗。
  - 排版優化: 支援 whitespace-pre-wrap 屬性，完整保留使用者輸入的換行與段落格式。
- **攻略收藏 (Guide)**:
  - Metadata 自動抓取: 貼上網址後自動獲取標題、封面圖與描述。
  - 簡約書籤樣式：方便快速瀏覽部落格、Dcard 攻略連結。
